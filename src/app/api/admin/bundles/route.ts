...
export async function GET(req: NextRequest) {
  ...
  if (!auth.ok) return auth.response; try { const bundles = await db.bundle.findMany({ orderBy: { createdAt: "desc" }, include: { items: { include: { product: true } } } }); return NextResponse.json({ ok: true, bundles }); } catch (e) { return NextResponse.json({ ok: false, error: e.message }, { status: 500 }); } }
