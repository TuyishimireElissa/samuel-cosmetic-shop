...
function CustomerModal({ customer, currency, onClose, onUpdated }: any) {
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{customer.name} — {customer.phone}</DialogTitle></DialogHeader>
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <div className="text-center py-10">{t("admin.loading", lang)}</div> : reviews.length === 0 ? <div className="text-center py-16 bg-white rounded-2xl border border-pink-100"><div className="text-6xl mb-2">⭐</div><p className="text-muted-foreground">{t("product.noReviews", lang)}</p></div> : (
        <div className="space-y-2">{reviews.map(r => (<div key={r.id} className="bg-white rounded-xl border border-pink-100 p-4"><div className="flex items-start justify-between gap-3 mb-2"><div><div className="flex items-center gap-2"><span className="font-semibold">{r.customerName}</span><span className="text-xs text-muted-foreground">{r.customerPhone}</span></div><div className="text-xs text-muted-foreground mt-0.5">{r.product?.nameEn} · {new Date(r.createdAt).toLocaleDateString()}</div></div><div className="flex gap-1">{[1,2,3,4,5].map(n => <Star key={n} size={14} className={n <= r.rating ? "text-amber-500 fill-amber-500" : "text-gray-300"} />)}</div></div>{r.title && <div className="font-medium text-sm mb-1">{r.title}</div>}<p className="text-sm text-foreground/80 mb-2">{r.body}</p><div className="flex gap-2 flex-wrap">{!r.isApproved && <Button size="sm" onClick={() => action(r.id, "approve")} className="bg-green-600 hover:bg-green-700 h-8"><Check size={12} className="mr-1" /> {t("admin.approve", lang)}</Button>}<Button size="sm" variant="outline" onClick={() => { setReplyTo(r); setReplyText(r.adminReply || ""); }} className="h-8 border-pink-200 text-pink-700"><Mail size={12} className="mr-1" /> {t("admin.messages.replyTo", lang)}</Button><Button size="sm" variant="ghost" onClick={() => action(r.id, "delete")} className="text-red-600 h-8" aria-label="Delete review"><Trash2 size={12} /></Button></div></div>))}</div>
      )}
    ...
    <div className="space-y-4">
      ...
      {loading ? <div className="text-center py-10">{t("admin.loading", lang)}</div> : (
        <><div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">{lang === "rw" ? "Byose" : lang === "fr" ? "Total" : "Total"}</div><div className="text-xl font-bold text-pink-700">{products.length}</div></CardContent></Card><Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">{lang === "rw" ? "Byarangije" : lang === "fr" ? "Épuisé" : "Out"}</div><div className="text-xl font-bold text-red-600">{products.filter(p => p.stockQty <= 0).length}</div></CardContent></Card><Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">{lang === "rw" ? "Byoroheje" : lang === "fr" ? "Faible" : "Low"}</div><div className="text-xl font-bold text-amber-600">{products.filter(p => p.stockQty > 0 && p.stockQty <= p.lowStockThreshold).length}</div></CardContent></Card><Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">{lang === "rw" ? "Agaciro" : lang === "fr" ? "Valeur" : "Value"}</div><div className="text-xl font-bold text-green-700">{formatPrice(products.reduce((s, p) => s + p.stockQty * p.costPrice, 0), currency)}</div></CardContent></Card></div>
          <div className="bg-white rounded-2xl border border-pink-100 overflow-x-auto"><table className="w-full text-sm"><thead className="bg-pink-50/50 text-pink-800"><tr><th className="text-left p-3">{t("admin.products", lang)}</th><th className="text-right p-3">{t("admin.inventory", lang)}</th><th className="text-right p-3">{t("admin.edit", lang)}</th></tr></thead><tbody>{products.map(p => (<tr key={p.id} className="border-t border-pink-50"><td className="p-3">{p.nameEn}</td><td className="p-3 text-right">{p.stockQty <= 0 ? <Badge variant="destructive" className="text-[10px]">OUT</Badge> : p.stockQty <= p.lowStockThreshold ? <Badge className="text-[10px] bg-amber-100 text-amber-800">{p.stockQty} ⚠</Badge> : <span>{p.stockQty}</span>}</td><td className="p-3 text-right"><Button size="sm" variant="outline" onClick={() => { setAdjusting(p); setNewQty(p.stockQty); }} className="border-pink-200 text-pink-700 h-8" aria-label={t("admin.edit", lang)}><Edit size={12} /></Button></td></tr>))}</tbody></table></div>
        </>
      )}
...
export function BundlesView() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <div className="text-center py-10">{t("admin.loading", lang)}</div> : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{bundles.map(b => (<Card key={b.id}><CardContent className="p-4"><div className="w-12 h-12 mb-2 rounded-lg bg-gradient-to-br from-pink-100 to-purple-100 grid place-items-center"><Gift size={24} className="text-pink-500" /></div><div className="font-bold text-pink-900">{b.nameEn}</div><div className="flex items-center gap-2 text-sm"><span className="font-bold text-pink-700">{formatPrice(b.bundlePrice, currency)}</span><span className="line-through text-muted-foreground">{formatPrice(b.normalPrice, currency)}</span><Badge className="bg-green-100 text-green-700 text-[10px]">-{b.savingsPct}%</Badge></div><div className="flex gap-1 mt-3"><Button size="sm" variant="outline" onClick={() => setEdit(b)} className="border-pink-200 text-pink-700 h-8 flex-1"><Edit size={12} className="mr-1" /> {t("admin.edit", lang)}</Button><Button size="sm" variant="ghost" onClick={() => del(b.id)} className="text-red-600 h-8" aria-label="Delete bundle"><Trash2 size={12} /></Button></div></CardContent></Card>))}</div>}
...
export function WholesaleAdminView() {
  ...
  async function approve(id: string) { const r = await safeFetch(`/api/admin/wholesale/${id}/approve`, { method: "PATCH" }); if (r.ok) { toast.success(lang === "rw" ? "Byemewe" : lang === "fr" ? "Approuvé" : "Approved"); setUsers(prev => prev.map(u => u.id === id ? { ...u, status: "approved" } : u)); } else toast.error(r.error); }
  async function doReject() { if (!rejectUser) return; if (!rejectReason.trim()) { toast.error(lang === "rw" ? "Andika impamvu" : lang === "fr" ? "Entrez une raison" : "Enter a reason"); return; } const r = await safeFetch(`/api/admin/wholesale/${rejectUser.id}/reject`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reason: rejectReason }) }); if (r.ok) { toast.success(lang === "rw" ? "Byanze" : lang === "fr" ? "Rejeté" : "Rejected"); setUsers(prev => prev.map(u => u.id === rejectUser.id ? { ...u, status: "rejected" } : u)); setRejectUser(null); setRejectReason(""); } else toast.error(r.error); }
  async function reject(id: string) { const u = users.find(x => x.id === id); setRejectUser(u || { id }); setRejectReason(""); }
  async function suspend(id: string) { const r = await safeFetch(`/api/admin/wholesale/${id}/suspend`, { method: "PATCH" }); if (r.ok) { toast.success(lang === "rw" ? "Byahagaritswe" : lang === "fr" ? "Suspendu" : "Suspended"); setUsers(prev => prev.map(u => u.id === id ? { ...u, status: "suspended" } : u)); } else toast.error(r.error); }
  return (
    <div className="space-y-4">
      ...
      {rejectUser && <Dialog open onOpenChange={() => { setRejectUser(null); setRejectReason(""); }}><DialogContent><DialogHeader><DialogTitle>{t("admin.reject", lang)} — {rejectUser.businessName || rejectUser.id}</DialogTitle></DialogHeader><div className="space-y-3"><Label>{lang === "rw" ? "Impamvu yo guhakana *" : lang === "fr" ? "Raison du rejet *" : "Rejection reason *"}</Label><Textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={3} className="bg-pink-50/50" placeholder={lang === "rw" ? "Andika impamvu..." : lang === "fr" ? "Entrez la raison..." : "Enter reason..."} /></div><DialogFooter><Button variant="outline" onClick={() => { setRejectUser(null); setRejectReason(""); }}>{t("admin.cancel", lang)}</Button><Button onClick={doReject} className="bg-red-600 hover:bg-red-700">{t("admin.reject", lang)}</Button></DialogFooter></DialogContent></Dialog>}
    </div>
  );
}
...
function CategoryForm({ category, onClose, onSaved }: any) {
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-lg">
