...
export function OrderTrackingModal({ onClose }: { onClose: () => void }) {
  ...
  async function track() { if (!phone) return; setLoading(true); setSearched(true); try { const res = await fetch(`/api/orders/track?phone=${encodeURIComponent(phone)}`); const data = await res.json(); if (data.ok) setOrders(data.orders); else toast.error(data.error || "Failed"); } catch { toast.error("Network error"); } finally { setLoading(false); } }
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Package size={20} className="text-pink-600" /> {t("modal.track.title", lang)}</DialogTitle></DialogHeader>
      ...
      {orders.length > 0 && <div className="space-y-3">{orders.map((o) => { let items: any[] = []; try { items = JSON.parse(o.itemsJson || "[]"); } catch { items = []; } const itemCount = items.reduce((s: number, i: any) => s + (i.qty || 0), 0); const currentStep = Math.max(0, statusFlow.indexOf(o.status)); return (
...
export function CustomerPortalModal({ onClose }: { onClose: () => void }) {
  ...
  const [loading, setLoading] = useState(false);
  async function lookup() { if (!phone) return; setLoading(true); try { const res = await fetch(`/api/customers/lookup?phone=${encodeURIComponent(phone)}`); const d = await res.json(); if (d.ok) setData(d); else setData({ error: d.error }); } catch { toast.error("Network error"); } finally { setLoading(false); } }
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Award size={20} className="text-pink-600" /> {t("modal.portal.title", lang)}</DialogTitle></DialogHeader>
...
export function BookingModal({ onClose }: { onClose: () => void }) {
  ...
  async function submit() { setLoading(true); try { const res = await fetch("/api/bookings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerName: name, customerPhone: phone, customerEmail: email, service, date, timeSlot, notes }) }); const data = await res.json(); if (data.ok) { setSuccess(data.booking); setStep(5); } else toast.error(data.error || "Booking failed"); } catch { toast.error("Network error"); } finally { setLoading(false); } }
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-lg max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Clock size={20} className="text-pink-600" /> {t("modal.booking.title", lang)}</DialogTitle></DialogHeader>
...
export function WholesaleModal({ onClose }: { onClose: () => void }) {
  ...
  async function register() { setLoading(true); setError(""); try { const res = await fetch("/api/wholesale/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const data = await res.json(); if (data.ok) { toast.success(lang === "rw" ? "Kwiyandikisha byagenze neza! Tegereza kwemezwa." : lang === "fr" ? "Inscription réussie ! En attente d'approbation." : "Registration successful! Await approval."); onClose(); } else setError(data.error); } catch { toast.error("Network error"); } finally { setLoading(false); } }
  async function login() { setLoading(true); setError(""); try { const res = await fetch("/api/wholesale/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tin: loginTin, password: loginPass }) }); const data = await res.json(); if (data.ok) { wholesaleLogin(data.token, data.user); setMode("dashboard"); } else setError(data.error); } catch { toast.error("Network error"); } finally { setLoading(false); } }
  const tierDiscounts: Record<string, number> = { bronze: 5, silver: 8, gold: 12, platinum: 18 };
  return (
    <Dialog open onOpenChange={onClose}><DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Package size={20} className="text-pink-600" /> {t("modal.wholesale.title", lang)}</DialogTitle></DialogHeader>
